 // سیستم مدیریت چندین فروشگاه - کد اصلی
        (function() {
            'use strict';
            ...
            const SystemState = {
                ...
                loadFromStorage() {
                    try {
                        // بارگذاری داده‌ها از sessionStorage (موقتی)
                        const savedUsers = sessionStorage.getItem('store_users');
                        const savedPending = sessionStorage.getItem('pending_approvals');
                        const savedCurrent = sessionStorage.getItem('current_user');
                ...
                saveToStorage() {
                    try {
                        sessionStorage.setItem('store_users', JSON.stringify(this.users));
                        sessionStorage.setItem('pending_approvals', JSON.stringify(this.pendingApprovals));
                        if (this.currentUser) {
                ...
                updateAdminStats() {
                    ...
                    // محاسبه فروشگاه‌های جدید (ایجاد شده در 7 روز گذشته)
                ...
                editStore(userId) {
                    const user = this.users.find(u => u.id === userId);
                    if (user) {
                        // در اینجا می‌توانید یک مودال برای ویرایش اطلاعات فروشگاه ایجاد کنید
